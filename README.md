...
**Super + Shift + R** é o atalho padrão; vale a combinação que você configurar. Cada acionamento faz **uma chamada ao Cloud Vision** e, se houver texto novo, no máximo uma chamada ao Cloud Translation. Textos conhecidos usam o cache de tradução, mas ainda passam pelo OCR online. Também é possível clicar em **Traduzir agora**. Não há tradução automática nem confirmação por três leituras. Enquanto um pedido estiver em andamento, novas pressões são agrupadas, sem criar fila. Se a captura estiver encerrada ou pausada, o atalho mostra uma notificação com a orientação necessária.
